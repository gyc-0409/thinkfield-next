import { NextResponse } from 'next/server';
import pool from '@/lib/db';
import { serverError } from '@/lib/auth';
import { formatExerciseTex, texResponse } from '@/lib/exportTex';
import {
  assertBookExportable,
  loadExerciseCommentTrees,
  parseIncludeComments,
} from '@/lib/exportTexServer';

export async function GET(request, { params }) {
  try {
    const { id: exerciseId } = await params;
    const includeComments = parseIncludeComments(new URL(request.url).searchParams);

    const result = await pool.query('SELECT * FROM exercises WHERE id = $1', [exerciseId]);
    if (result.rowCount === 0) {
      return NextResponse.json({ error: '习题不存在' }, { status: 404 });
    }
    const exercise = result.rows[0];
    const gate = await assertBookExportable(exercise.book_id);
    if (gate.error) return NextResponse.json({ error: gate.error }, { status: gate.status });

    const answers = Array.isArray(exercise.answers) ? exercise.answers : [];
    const commentsByAnswerId = includeComments
      ? await loadExerciseCommentTrees(exerciseId, answers)
      : {};

    const tex = formatExerciseTex(exercise, commentsByAnswerId, { includeComments });
    const filename = `习题_${exercise.title || exerciseId}.tex`;
    return texResponse(tex, filename);
  } catch (e) {
    return serverError(e, 'export/exercise GET');
  }
}
