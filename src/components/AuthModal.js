...
export default function AuthModal({ onClose }) {
  ...
  const handleSendCode = async () => {
    ...
    try {
      const url = isReset ? '/api/reset-password' : '/api/send-code';
      ...
      alert(data.message || '验证码已发送，5分钟内有效');
      setCodeCountdown(60);
    } catch (e) {
      alert(e.message);
    }
  ...
  const handleResetPassword = async () => {
    ...
    if (!code) return setError('请输入验证码');
    if (!resetPassword) return setError('请输入新密码');
    if (resetPassword.length < 8) return setError('密码至少8位');
  ...
  const title = isReset ? '重置密码' : isRegister ? '注册' : '登录';
      ...
      <div className="bg-white rounded-none md:rounded-lg p-6 md:p-8 w-full h-full md:h-auto md:max-w-md md:max-h-[90vh] overflow-y-auto relative flex flex-col">
        ...
        {isReset ? (
          ...
          <>
            ...
            {!isRegister && (
              <p
                className="text-center mt-3 text-sm text-gray-400 hover:text-gray-600 cursor-pointer transition-colors"
                onClick={() => switchStep('reset')}
              >
                忘记密码？
              </p>
            )}
            ...
            <p
              className="text-center mt-4 text-sm text-gray-400 hover:text-gray-600 cursor-pointer transition-colors"
              onClick={() => switchStep(isRegister ? 'login' : 'register')}
            >
