...
export async function DELETE(request, { params }) {
  ...
  try {
    const result = await pool.query('SELECT author, parent_id FROM exercise_comments WHERE id = $1', [commentId]);
    ...
    if (children.rowCount > 0) {
      ...
      while (parentId) {
        ...
        const parentChildren = await pool.query('SELECT id FROM exercise_comments WHERE parent_id = $1', [parentId]);
